...
def main(argv):
    ...
    if path_to_R == None:
        print 'Error: You must input a source/sink vector using the -R flag'
        sys.exit(2)
    if not os.path.exists(path_to_R):
        print 'Error: The file '+path_to_R+' does not exist'
        sys.exit(2)
    if   '.csv' in path_to_R:
        R = np.loadtxt(path_to_R, delimiter=',')
    elif '.npy' in path_to_R:
        R = np.load(path_to_R)
    else:
        print 'Error: The source/sink vector R must end in ".npy" or ".csv"'
        sys.exit(2)
