...
def main(argv):
    ...
    if path_to_S == None:
        print 'Error: You must input a lineage-specific sink matrix using the -S flag'
        sys.exit(2)
    if not os.path.exists(path_to_S):
        print 'Error: The file ' + path_to_S + ' does not exist'
        sys.exit(2)
    if '.csv' in path_to_S:
        S = np.loadtxt(path_to_S, delimiter=',')
    elif '.npy' in path_to_S:
        S = np.load(path_to_S)
    else:
        print 'Error: The lineage-specific sink matrix S must end in ".npy" or ".csv"'
        sys.exit(2)

    if path_to_V == None:
        tmp_path_to_V = '/'.join(path_to_S.split('/')[:-1] + ['V.npy'])
        if os.path.exists(tmp_path_to_V):
            V = np.load(tmp_path_to_V)
        else:
            print 'Error: You must create the file ' + tmp_path_to_V + '. Use "compute_potential.py"'
            sys.exit(2)
    else:
        if not os.path.exists(path_to_V):
            print 'The file ' + path_to_V + ' does not exist'
            sys.exit(2)
        else:
            V = np.load(path_to_V)

    if path_to_edge_list == None:
        tmp_path_to_edge_list = '/'.join(
            path_to_S.split('/')[:-1] + ['edge_list.csv'])
        if os.path.exists(tmp_path_to_edge_list):
            edges = load_edge_list(tmp_path_to_edge_list)
        else:
            print 'Error: You must create the file ' + tmp_path_to_edge_list + '. Use "compute_knn_graph.py"'
            sys.exit(2)
    else:
        if not os.path.exists(path_to_edge_list):
            print 'The file ' + path_to_edge_list + ' does not exist'
            sys.exit(2)
        else:
    ...
    bigP = np.hstack((P, S))
    bigP = np.vstack(
        (bigP, np.hstack((np.zeros((S.shape[1], P.shape[1])), np.identity(S.shape[1])))))
