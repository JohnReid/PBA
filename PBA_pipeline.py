...
def main(argv):
    ...
    for o, a in opts:
        if o == '-X':
            path_to_expression_matrix = a
        if o == '-E':
            minimum_mean_expression = float(a)
        if o == '-V':
            minimum_CV = float(a)
        if o == '-N':
            normalize = (a == 'True')
    ...
    if path_to_expression_matrix == None and path_to_edge_list == None:
        print 'Error: You must input either an expression matrix (-X) or knn edge list (-e)'
        sys.exit(2)
    elif path_to_edge_list == None:
        print '\n## Running compute_knn_graph.py'
        os.system('python compute_knn_graph.py -E ' + repr(minimum_mean_expression) + ' -V ' + repr(minimum_CV)
                  + ' -k ' + repr(k) + ' -p ' + repr(p) + ' -X ' + path_to_expression_matrix + ' -N ' + repr(normalize))
        path_to_edge_list = '/'.join(
            path_to_expression_matrix.split('/')[:-1] + ['edge_list.csv'])
    ...
    os.system('python compute_fate_probabilities.py  -S ' +
              path_to_S + ' -e ' + path_to_edge_list + ' -D ' + repr(D))
    ...
    if '.csv' in path_to_R:
        ...
        R = np.load(path_to_R)
    N_cells = len(R)
    command = 'python compute_mean_first_passage_times.py  -R ' + \
        path_to_R + ' -e ' + path_to_edge_list + ' -D ' + repr(D)
