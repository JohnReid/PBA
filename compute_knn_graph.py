...
def main(argv):
    ...
    if path_to_expression_matrix == None:
        print 'Error: You must input an expression matrix using the -X flag'
        sys.exit(2)
    if not os.path.exists(path_to_expression_matrix):
        print 'Error: The file '+path_to_expression_matrix+' does not exist'
        sys.exit(2)
    if   '.csv' in path_to_expression_matrix:
        E = np.loadtxt(path_to_expression_matrix, delimiter=',')
    elif '.npy' in path_to_expression_matrix:
        E = np.load(path_to_expression_matrix)
    else:
        print 'Error: The expression file must end in ".npy" or ".csv"'
        sys.exit(2)
    ...
    if np.sum(gene_filter) == 0:
        print 'Error: No genes survived filtering, consider lowering the minimum mean expression (-E) or minimum CV (-V)'
